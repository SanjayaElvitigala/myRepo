...
class Customer:
  ...
  def __init__(self, id_num):
    # self.segment = random.choices(self.customer_segment, weights= customer_segment_weights, k=1)[0] # adding customer segment according to weights in synthetic dataset
    self.segment = random.choice(self.customer_segment)
    ...
    self.room_value=round(np.random.normal(room_type[self.type]['cus_will_pay'][0],room_type[self.type]['cus_will_pay'][1],1)[0],2) * self.room_value_expected
    
    self.hotel_name = np.nan
    ...
    self.actual_gap = np.nan
    self.remaining_days = np.nan
    ...
    self.cancelled_day = np.nan
    self.stay_day_passsed_or_not=0
    self.trying_again=random.getrandbits(1)
