...
class Customer_Handler:
    ...
    def __init__(self,n_customers, simulation_days, holiday_df):
        ...
        self.df_holiday = holiday_df # assigning the holiday dataframe
        
        self.year_customers = []
        self.daily_customers = {i:[] for i in self.simu_days}
        self.daily_search_day={i:[] for i in self.simu_days}
    ...
    def modify_holiday_dataframe(self):
        ...
        month_daily_lamda = pd.DataFrame({'month':[i for i in range(1,13)], 'base_lambda': [self.n_customers/7000 for i in range(1,13)]}) 
        ...
        for i in df.index:
            ...
            for col in weight_cols:
                if current_row[col]!=0:
                    ...
                    day_exists, day_index, day =self.cust_buildup_start_day(current_row['day_index'],int(current_row[col]))
                    if day_exists:
                        start_day_index = day_index
                        for j in range(start_day_index,i + 1):
                            previous_row =df.iloc[j]
                            df[lambda_cols[weight_cols.index(col)]].iloc[j]= previous_row.base_lambda*( 1+ int(current_row[col]) )
                    else:
                        continue
                else:
                    continue
    ...
    def cust_buildup_start_day(self, current_day, weight):
        current_day_index = self.simu_days.index(current_day)
        index_exists = True if current_day_index-weight>0 else False
        buildup_day = self.simu_days[current_day_index-weight] if index_exists else -1
        return (index_exists, current_day_index-weight, buildup_day)
